...
class SubHeader {
	constructor() {
		...
		this.content.innerHTML = `
            ...
            <div class = "${styles["sub-header"]}  ${
			styles["max-width-wrapper"]
		}">
                <div class = "${
									styles.heading
								}">Inspiration for your first order</div>
                ...
                <div class = "${styles["food-list"]} food-list">
                    ${PICTURES.map((item, index) => {
											return `
                            <nav id = "food${index}" class = "${styles["food-item"]} food-item">
		...
		this.content
			...
			.addEventListener("click", (e) => {
				...
				document.querySelector(
					...
					(this.currentFoodItem + 1) *
					(-Number(current.offsetWidth) -
						Number(style.marginRight.slice(0, -2)))
				}px)`;
